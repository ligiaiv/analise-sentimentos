	...
	for word in word_vector:
		...
		if word not in STOPWORDS:
			if word in word_dict:
				...
				word_dict[word] = 1
				# print('--:',word)
			if word in word_dict_in_phrase:
				word_dict_in_phrase[word] += 1
			else:
				word_dict_in_phrase[word] = 1
			phrase_in_IQ+=[list(word_dict.keys()).index(word)]
			phrase_in_IQ+=[word_dict[word]]
			phrase_in_model[list(word_dict.keys()).index(word)]=word_dict_in_phrase[word]
	# return phrase_in_IQ
...
for phrase in tqdm(phrase_list,total=len(phrase_list)):
	...
	for index in phrase:
		if index not in little_ocourrences:
			new_line +=[index]
			new_line +=[phrase[index]]
			old_line[index] = phrase[index]
